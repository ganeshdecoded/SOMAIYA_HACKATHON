...
import com.example.medicinereminder.databinding.ActivityExerciseBinding
...
//        }
        ...
        binding?.btnNext?.setOnClickListener {
            ...
            if (currentExercisePosition == exerciseList!!.size - 1) {
                ...
                currentExercisePosition++ // Increment position
                exerciseList!![currentExercisePosition].setIsSelected(true) // Select the new exercise
                exerciseAdapter!!.notifyDataSetChanged() // Notify adapter of changes
...
//        }
        ...
        binding?.btnPlayPause?.setOnClickListener {
            if (isPaused) {
                ...
                isPaused = false
                binding?.btnPlayPause?.setBackgroundResource(R.drawable.pause)
                setExerciseProgressBar(pauseOffsetExercise) // Resume from where it was paused.
            } else {
                // Pause the timer
                isPaused = true
                binding?.btnPlayPause?.setBackgroundResource(R.drawable.play)
//                exerciseTimer?.cancel() // Stop the timer.
            }
            btnIsPlay = !btnIsPlay
        }
...
//        }
    ...
    private fun setRestProgressBar(pauseOffsetL: Long) {
        ...
        restTimer?.cancel() // Cancel any existing timer before starting a new one.

        restProgress = 0 // Reset the progress to 0.
        binding?.progressBar?.progress = restProgress

        restTimer = object : CountDownTimer(10000 - pauseOffsetL, 1000) {
            //            override fun onTick(millisUntilFinished: Long) {
//                val timeLeft = maxOf(0, millisUntilFinished / 1000) // Ensure no negative values.
//                binding?.tvTimerEx?.text = timeLeft.toString()
//
//                restProgress++
//
//                // Play Boop sound
...
//            }
            override fun onTick(millisUntilFinished: Long) {
                ...
                binding?.progressBar?.progress = timeLeft.toInt()
    ...
    private fun setExerciseProgressBar(pauseOffsetL: Long) {
        ...
        // Calculate remaining time based on the pause offset
        val remainingTime = 30000 - pauseOffsetL

        // Set the progress bar to reflect the remaining time
        binding?.ProgressBarEx?.progress = remainingTime.toInt()
        binding?.tvTimerEx?.text = (remainingTime / 1000).toString() // Update UI with remaining time

        exerciseTimer = object : CountDownTimer(remainingTime, 1000) {
//            override fun onTick(millisUntilFinished: Long) {
//                exerciseProgress++
//
//                // Play Boop sound
...
//            }
            ...
            override fun onTick(millisUntilFinished: Long) {
                if (isPaused) {
                    pauseOffsetExercise = 30000 - millisUntilFinished
                    cancel() // Cancel the timer if paused.
                    return
                }

                val timeLeft = millisUntilFinished / 1000
                binding?.ProgressBarEx?.progress = timeLeft.toInt()
                binding?.tvTimerEx?.text = timeLeft.toString()
            }
            ...
            override fun onFinish() {
                ...
                if (currentExercisePosition < exerciseList?.size!! - 1) {
                    ...
                    finish()
                    val intent =
                        Intent(this@ExerciseActivity, WorkoutCompletionActivity::class.java)
                    intent.putExtra("totalExerciseDone", totalExerciseDone)
                    startActivity(intent)
                }
            }
...
//    }
    ...
    private fun pauseTimer(isRest: Boolean) {
        if (isRest) {
            restTimer?.cancel()
        } else {
            exerciseTimer?.cancel()
